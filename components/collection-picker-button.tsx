"use client";

import { useState } from "react";
import { Bookmark } from "lucide-react";
import { useAccount } from "@/components/account-provider";

export function CollectionPickerButton({ predictionId }: { predictionId: number }) {
  const { user, collections, toggleCollectionItem, createCollection } = useAccount();
  const [open, setOpen] = useState(false);
  const [newTitle, setNewTitle] = useState("");
  const [creating, setCreating] = useState(false);

  const inAnyCollection = collections.some((c) => c.predictionIds.has(predictionId));

  async function handleCreate() {
    if (!newTitle.trim()) return;
    setCreating(true);
    const id = await createCollection(newTitle);
    setCreating(false);
    setNewTitle("");
    if (id !== null) toggleCollectionItem(id, predictionId);
  }

  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-label="Save to a collection"
        className="inline-flex size-6 shrink-0 items-center justify-center rounded-md border border-border/60 hover:bg-muted/60"
      >
        <Bookmark className={inAnyCollection ? "size-3.5 fill-primary text-primary" : "size-3.5"} />
      </button>

      {open && (
        <>
          <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} />
          <div className="absolute left-0 top-full z-20 mt-1 w-56 rounded-md border border-border/60 bg-popover p-2 shadow-lg">
            {!user ? (
              <p className="p-2 text-xs text-muted-foreground">Log in to save matches to a collection.</p>
            ) : (
              <>
                {collections.length === 0 && (
                  <p className="px-1 pb-2 text-xs text-muted-foreground">No collections yet.</p>
                )}
                <div className="max-h-40 space-y-0.5 overflow-y-auto">
                  {collections.map((c) => (
                    <label
                      key={c.id}
                      className="flex cursor-pointer items-center gap-2 rounded px-1.5 py-1.5 text-sm hover:bg-muted"
                    >
                      <input
                        type="checkbox"
                        checked={c.predictionIds.has(predictionId)}
                        onChange={() => toggleCollectionItem(c.id, predictionId)}
                        className="size-3.5"
                      />
                      <span className="truncate">{c.title}</span>
                    </label>
                  ))}
                </div>
                <div className="mt-2 flex items-center gap-1 border-t border-border/60 pt-2">
                  <input
                    value={newTitle}
                    onChange={(e) => setNewTitle(e.target.value)}
                    placeholder="New collection"
                    className="h-7 flex-1 rounded-md border border-border/60 bg-background px-2 text-xs outline-none"
                  />
                  <button
                    type="button"
                    onClick={handleCreate}
                    disabled={creating || !newTitle.trim()}
                    className="h-7 shrink-0 rounded-md bg-primary px-2 text-xs font-medium text-primary-foreground disabled:opacity-60"
                  >
                    Add
                  </button>
                </div>
              </>
            )}
          </div>
        </>
      )}
    </div>
  );
}
